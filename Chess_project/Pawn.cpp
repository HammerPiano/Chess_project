...
Pawn::Pawn(Point coordinates, bool white) : Piece(coordinates,"Pawn",white)
{
...
{
	char yDiff = abs(coordinates.getY() - this->_coordinates.getY());
	char xDiff = abs(coordinates.getX() - this->_coordinates.getX());
	...
	else if (xDiff == 1 && yDiff == 1 && dstPiece != nullptr && dstPiece->isWhite() != this->isWhite())
	{
		this->_coordinates = coordinates;
	}
	else
